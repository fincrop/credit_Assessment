...
import { VEGETATION_LINE } from '../../lib/vizPalette';
function StatCard({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="bg-paper rounded-lg border border-rule p-3">
      <p className="text-[10px] font-semibold text-ink-muted uppercase tracking-widest mb-1.5">{label}</p>
...
function CropsList({ ca }: { ca: CroppingAnalysis }) {
  ...
  if (!raw) return <p className="text-sm text-ink-muted">No crop list.</p>;
  ...
  if (entries.length === 0) return <p className="text-sm text-ink-muted">No crops detected.</p>;
...
function MiniSparkline({ label, values }: { label: string; values: number[] }) {
  ...
  return (
    <div className="bg-paper rounded-lg border border-rule p-3">
      <p className="text-[10px] font-semibold text-ink-muted uppercase tracking-widest mb-2">{label}</p>
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-12" preserveAspectRatio="none">
        <defs>
          <linearGradient id={`grad-${label}`} x1="0" y1="0" x2="0" y2="1">
            {/* Vertical fade only. The value is the polyline's y-position, so
                the gradient never runs along an axis that encodes anything. */}
  ...
  if (!ca && !stats) {
    return (
      <div className="bg-white rounded-xl border border-rule p-6">
        ...
        <p className="text-sm text-ink-muted">No cropping analysis in this payload.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl border border-rule p-6 space-y-5">
      ...
      {ca && (
        <>
          ...
          <div>
            <p className="text-[11px] font-semibold text-ink-muted uppercase tracking-wider mb-3">Crops Detected</p>
            <CropsList ca={ca} />
          </div>

          {series.length > 2 && (
            <div>
              <p className="text-[11px] font-semibold text-ink-muted uppercase tracking-wider mb-3">Vegetation Index Trends</p>
