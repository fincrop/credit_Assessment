...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('✅ Farmers Webhook response saved:', result.insertedId);
        ...
        console.error('❌ Farmers Webhook Error:', error);
        return NextResponse.json(
            {
                success: false,
...
export async function GET() {
    return NextResponse.json({
        status: 'active',
        endpoint: '/webhook/farmers/on-seek',
        message: 'Webhook is listening for AgriStack Farmers responses',
    });
}
