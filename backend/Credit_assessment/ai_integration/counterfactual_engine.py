...
class CounterfactualEngine:
    ...
    def generate(self, assessment: Dict) -> Dict:
        ...
        if isinstance(ra, dict) and ra.get('sub_indices'):
            ...
            scenarios = self._generate_scenarios_v5(ra)
        else:
            cr              = assessment.get('credit_assessment', {})
            current_score   = float(cr.get('credit_score', 50))
            current_risk = cr.get('risk_category', 'UNKNOWN')
    ...
    # v5: actionable scenarios derived directly from the risk-index sub-indices.
    _V5_ACTIONS = {
        'landuse': (
            "Raise cropping intensity",
            "Add a rabi/zaid cycle or reduce fallow gaps between cycles to lift the "
            "Land-Use & Activity sub-index.",
        ),
        'vigor': (
        ...
        'stability': (
            ...
            "Timely irrigation and pest/disease control around flowering & grain-fill to "
            "cut stress anomalies and year-to-year variability.",
        ),
        'weather': (
            "Build weather resilience",
            "Supplemental/backup irrigation, drought-tolerant varieties and crop-insurance "
            "linkage to improve resilience and buffer exposure.",
        ),
    }

    def _generate_scenarios_v5(self, ra: Dict) -> List[Dict]:
        """
        Weight-aware scenarios from the v5 sub-indices. The index gain from raising
        a sub-index from its current score s to target t is
        (t - s) * weight/100 * confidence_gate  (data_confidence is not farmer-actionable).
        """
        subs = ra.get('sub_indices', {}) or {}
        weights = ra.get('weights', {}) or {}
        gate = float(ra.get('confidence_gate', 1.0))
        scenarios: List[Dict] = []
        idx = 1
        for comp, (title, action) in self._V5_ACTIONS.items():
            ...
            gain = round(max(0.0, (target - s) * (w / 100.0) * gate), 1)
            if gain <= 0:
                continue
            gap = target - s
            feasibility = 'HIGH' if gap <= 15 else ('MEDIUM' if gap <= 30 else 'LOW')
            scenarios.append({
                'id': f'cf_v5_{idx}',
                'title': title,
                'component': comp,
                'current_component_score': round(s, 1),
                'target_component_score': round(target, 1),
                'score_gain': gain,
